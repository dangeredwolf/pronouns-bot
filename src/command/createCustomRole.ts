...
import { OptionedCommandInteraction, PronounNames, Pronouns, DefaultPronounNames } from '../types';
import { CommandFailed, getErrorString } from '../errors';

export const CreateCustomPronounCommand = async (interaction: OptionedCommandInteraction) => {
  ...
  if (DefaultPronounNames[pronounName] === true) {
    throw new CommandFailed(Strings.CUSTOM_ROLE_IS_DEFAULT.format({ pronoun: pronounName }));
  }
